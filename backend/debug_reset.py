...
async def run():
    async with async_session() as session:
        queries = [
            ...
            "UPDATE audit_logs SET actor_id = NULL WHERE actor_id NOT IN (SELECT id FROM users)",
            "UPDATE scan_logs SET scanned_by = NULL WHERE scanned_by NOT IN (SELECT id FROM users)",
            "UPDATE system_activities SET user_id = NULL WHERE user_id NOT IN (SELECT id FROM users)",
